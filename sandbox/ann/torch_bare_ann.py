...
class NeuralNet_By_Torch(object):
    ...
    def __init__(self, X, Y, batch_size: int = None, hidden_layers: [int] = []):
        ...
        yy = torch.clone(Y) if Y.ndim > 1 else torch.unsqueeze(Y, dim=1)
        batch_X: [] = torch.split(X, batch_cnt)
        batch_Y: [] = torch.split(yy, batch_cnt)
        ...
        for neuron_cnt in h_layers:
            ...
            ww_bias = torch.randn(neuron_cnt)
            self.W.append(ww)
    ...
    def iteration_predict(self):
        x_train = torch.cat(([ite[0] for ite in self.batch_X_Y]), dim=0)
    ...
    def __backward(self, X, Y, H_out):
        ...
        for idx in range(self.h_layer_cnt-1, 0, -1):
            #self.W[idx] += (1 / self.train_cnt) * self.eta * H_out[idx-1].T @ delta_H[idx]
            self.W[idx] += H_out[idx-1].T @ delta_H[idx]
            self.W_bias[idx] += vec_one @ delta_H[idx]
        # update weights: at layer W0 back to input
        #self.W[0] += (1 / self.train_cnt) * self.eta * self.X.T @ delta_H[0]
