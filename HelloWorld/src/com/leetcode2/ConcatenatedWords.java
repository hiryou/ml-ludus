...
public class ConcatenatedWords {
    ...
    public static void main(String[] args) {
        ...
        BenchMark.run(() -> p.findAllConcatenatedWordsInADict(words));
    }

    Trie root = new Trie();

    class Trie {
        Trie[] child = new Trie[26]; // 'a' to 'z'
        boolean isWord = false; // true if marking the end of a word

        boolean addWord(String w) {
            // check if this word can be used by any existing words with shorter lengths
            boolean useExisting = traceChar(w, 0);
            // if not, add it as a new word
            if (!useExisting) {
                this.addChar(w, 0);
            }
            return useExisting;
        }
        private void addChar(String w, int idx) {
            // stop condition: marking end of a word
            if (idx==w.length()) {
                this.isWord = true;
                return;
            }
            ...
            child[c - 'a'].addChar(w, idx+1);
        }

        private boolean traceChar(String w, int idx) {
            // stop condition of finding a success or not
            if (idx==w.length()) {
                return this.isWord;
            }

            boolean ans = false;
            // if reached a word, another option is starting from root again
            if (this.isWord) ans |= root.traceChar(w, idx);
            if (ans) return ans;

            // or normally, just keep going further
            char c = w.charAt(idx);
            if (child[c - 'a']==null) return false;
            ans |= child[c - 'a'].traceChar(w, idx+1);
            return ans;
        }
    }

    public List<String> findAllConcatenatedWordsInADict(String[] ws) {
        TreeMap<Integer, List<String>> lenWords = new TreeMap<>();

        for (String w: ws) {
            int len = w.length();
            if (!lenWords.containsKey(len)) lenWords.put(len, new LinkedList<>());
            lenWords.get(len).add(w);
        }

        List<String> res = new LinkedList<>();
        for (Map.Entry<Integer, List<String>> e: lenWords.entrySet()) {
            List<String> words = e.getValue();
            for (String w: words) if (!w.equals("")) {
                boolean check = root.addWord(w);
                if (check) res.add(w);
            }
        }
